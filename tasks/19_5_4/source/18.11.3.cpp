...
Link* Link::advance()
{
	...
	if (!(p->succ) && (p->value <= system_length && p->level < system_levels))
	{
		p = p->to_first();
		if (p->upper)
		{
...
// Всё работает, осталось лишь научить программу включать элементы между значениями 1 и 100 (увеличить длину системы)
